...
namespace libfreenect2
{
    ...
    void RegistrationImpl::depth_to_color(float mx, float my, float& rx, float& ry) const
    {
        ...
        float wx = (mx * mx * mx * color.mx_x3y0) + (my * my * my * color.mx_x0y3) +
                   (mx * mx * my * color.mx_x2y1) + (my * my * mx * color.mx_x1y2) + (mx * mx * color.mx_x2y0) +
                   (my * my * color.mx_x0y2) + (mx * my * color.mx_x1y1) + (mx * color.mx_x1y0) +
                   (my * color.mx_x0y1) + (color.mx_x0y0);

        float wy = (mx * mx * mx * color.my_x3y0) + (my * my * my * color.my_x0y3) +
                   (mx * mx * my * color.my_x2y1) + (my * my * mx * color.my_x1y2) + (mx * mx * color.my_x2y0) +
                   (my * my * color.my_x0y2) + (mx * my * color.my_x1y1) + (mx * color.my_x1y0) +
                   (my * color.my_x0y1) + (color.my_x0y0);
        ...
        int* color_depth_map) const
    {
        ...
        if(!rgb || !depth || !undistorted || !registered || rgb->width != 1920 || rgb->height != 1080 ||
           rgb->bytes_per_pixel != 4 || depth->width != 512 || depth->height != 424 ||
           depth->bytes_per_pixel != 4 || undistorted->width != 512 || undistorted->height != 424 ||
           undistorted->bytes_per_pixel != 4 || registered->width != 512 || registered->height != 424 ||
        ...
        // size of filter map with a border of filter_height_half on top and bottom so that no check for borders is
        // needed. since the color image is wide angle no border to the sides is needed.
        ...
        // the four maps have the same structure as the images, so their pointers are increased each iteration as
        // well
        for(int i = 0; i < size_depth; ++i, ++undistorted_data, ++map_dist, ++map_x, ++map_yi, ++map_c_off)
        {
            ...
            *map_c_off = c_off;
            ...
            if(enable_filter)
            {
                // setting a window around the filter map pixel corresponding to the color pixel with the current z
                // value
                int yi = (cy - filter_height_half) * 1920 + cx - filter_width_half; // index of first pixel to set
                for(int r = -filter_height_half; r <= filter_height_half;
                    ++r, yi += 1920) // index increased by a full row each iteration
                {
    ...
    {
        // Check if all frames are valid and have the correct size
        if(!depth || !undistorted || depth->width != 512 || depth->height != 424 || depth->bytes_per_pixel != 4 ||
    ...
    void Registration::getPointXYZRGB(
        const Frame* undistorted, const Frame* registered, int r, int c, float& x, float& y, float& z, float& rgb)
        const
    {
        impl_->getPointXYZRGB(undistorted, registered, r, c, x, y, z, rgb);
    }

    void RegistrationImpl::getPointXYZRGB(
        const Frame* undistorted, const Frame* registered, int r, int c, float& x, float& y, float& z, float& rgb)
        const
    {
    ...
    void RegistrationImpl::getPointXYZ(const Frame* undistorted, int r, int c, float& x, float& y, float& z) const
    {
        ...
        const float depth_val =
            undistorted_data[512 * r + c] / 1000.0f; // scaling factor, so that value of 1 is one meter.
    ...
    RegistrationImpl::RegistrationImpl(
        Freenect2Device::IrCameraParams depth_p, Freenect2Device::ColorCameraParams rgb_p)
